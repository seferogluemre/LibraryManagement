import { UpdateShipyardForm } from "#components/form/update/update-shipyard.js";
...
const RowActions = ({ row }: { row: { original: CompanyShowResponse } }) => {
  ...
  const handleEdit = () => {
    openForm(
      ...
      (onClose) => (
        <UpdateShipyardForm
          shipyardData={row.original}
          onClose={onClose}
          onSuccess={() => {
            // Refresh the shipyards list
            queryClient.invalidateQueries({ queryKey: ["shipyards"] });
            onClose();
          }}
        />
      ),
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.error) {
        toast({
          title: "Error",
          description: "Failed to delete shipyard",
          variant: "destructive",
        });
        return;
      }
      ...
      toast({
        title: "Success",
        description: "Shipyard deleted successfully",
      });
...
export const shipyardColumns: ColumnDef<CompanyShowResponse, any>[] = [
